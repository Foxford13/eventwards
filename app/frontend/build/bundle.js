...
"use strict";
/* WEBPACK VAR INJECTION */(function($) { // eslint-disable-line
...
$(document).ready(function () {
	...
	function slideBoxes(sliderDuration, direction) {
		...
		var $marginOfSlidedElement = parseInt($(this).siblings('.card-list').children('.card').first().css('margin-right'));

		var $widthOfSlidedElement = $(this).siblings('.card-list').children('.card').first().outerWidth() + $marginOfSlidedElement;

		var $view = $(this).siblings('.card-list');
		var currentPosition = $view[0].scrollLeft;
		var curBoxShown;
		var nextBoxToShow;
		var scrollLeft;
	...
	function disapearingArrows() {
		...
		var $element = $(this);
		var leftScroll = $element[0].scrollLeft;
		var widthScroll = $element[0].scrollWidth;
		var widthOffset = $element[0].offsetWidth;
		var arrowLeft = $element.siblings('.leftArrow');
		var arrowRight = $element.siblings('.rightArrow');
		var ifExpresstionScroll = Math.ceil(leftScroll) === widthScroll - widthOffset;
	...
	function activateArrows(arrowsArg, source) {
		...
		arrowsArg.each(function (index, element) {

			var $element = $(element);
			var type = $element.hasClass('leftArrow') ? 'left' : 'right';
	...
	setTimeout(function () {
		$('.rightArrow').each(function (index, element) {
			...
			if ($(element).closest('.arrow-container').find('.card-list')[0].scrollWidth <= $(element).closest('.arrow-container').find('.card-list').width()) {
				console.log('invisible right arrrow!!!!!\xB1!!');
 ...
 */
	var $modal = $('.modal');
	var $openPopupSpeaker = $('.openpop');
	var $openPopContact = $('.openpop-con');
	var $openPopVideo = $('.openpop-video');
	var $closePopBtn = $('.button-close, .close-modal');
	...
	function appendMoreLessText(summaryText, readMoreText, targetDiv) {
		...
		arr.push('<p class="read-more-wrap">' + summaryText + '<span class="ellipsis">...</span><span class="read-more-target">' + readMoreText + '</span><label for="post-1" class="read-more-trigger">more+</label></p>');

		targetDiv.append(arr);

		var readMoreBtn = $('.read-more-trigger');
	...
	function modifyTextOnPop(speakerId, data) {
		...
		setTimeout(function () {
			activateArrows($('.modal .leftArrow, .modal .rightArrow'));
		}, 0);
 ...
 */
	...
	$openPopContact.on('click', function () {
	...
	$('.contact-organiser-btn').on('click', function () {
	...
	$openPopVideo.on('click', function (event) {

		var videoId = event.target.closest('.openpop-video').id.replace(/^\D+/g, '');
	...
	function readMore(targetDiv) {
		...
		var text = targetDiv.html();

		var summaryText = void 0;
		var readMoreText = void 0;
 ...
 */
	...
	$('.takeaways-trigger').on('click', function () {
	...
	function appendErrorFormVal(data) {
		...
		if (data.success === false) {
			$('.validation-message').empty().append('\t<div class="error-message message "><i class="fa fa-exclamation-triangle" aria-hidden="true">\n\t\t\t\t\t</i>Please fill out the form correctly<span><i class="fa fa-times" aria-hidden="true"></i></span>\n\t\t\t\t\t</div>');
		} else {
			$('.validation-message').empty().append('<div class="success-message message ">\n\t\t\t\t\t\t<i class="fa fa-check" aria-hidden="true"></i>Your message has been sent<span>\n\t\t\t\t\t\t<i class="fa fa-times" aria-hidden="true"></i>\n\t\t\t\t\t\t</span></div>');
		}
	...
	module.exports = {
		...
		disapearingArrows: disapearingArrows,
		ajaxTemplate: ajaxTemplate,
		appendErrorFormVal: appendErrorFormVal
	};
});
...
(function () {
  ...
  s1.async = true;
  s1.src = 'https://embed.tawk.to/5a1c3401198bd56b8c03d9c6/default';
  s1.charset = 'UTF-8';
...
$(document).ready(function () {
	var _require = __webpack_require__(32),
	    ajaxTemplate = _require.ajaxTemplate;

	$('.new-video').on('submit', function (event) {
		event.preventDefault();

		ajaxTemplate('/admin-videos/new', 'POST', newVideoData($('.new-video')), successVideo, 'json');
	});

	$('.edit-video').on('submit', function (event) {
		event.preventDefault();
	...
	$('.btn-delete-video').on('click', function (event) {

		var videoDeleteId = event.target.id;
...
$(document).ready(function () {
	var _require = __webpack_require__(32),
	    ajaxTemplate = _require.ajaxTemplate; // eslint-disable-line
	...
	function collectSponsorsObject(collectClass) {
		...
		var arrayOfSponsors = [];
		var name = document.getElementsByClassName(collectClass + '-name');
		var fileName = document.getElementsByClassName(collectClass + '-filename');
		var url = document.getElementsByClassName(collectClass + '-url');
	...
	function collectSessions() {
		...
		var timeStart = $('.session-time-from');
		var timeFinish = $('.session-time-to');
	...
	function collectTakeaways() {
		...
		var arrayOfTakeaways = [];
		var takeaways = $('.takeaways-value');

		for (var index = 0; index < takeaways.length; index++) {
 ...
 */
	...
	$('.btn-delete-event').on('click', function (event) {
		var eventDeleteId = event.target.id;
 ...
 */
	...
	function appendSponsorsInitialize() {
		$('.plat-content-btn').on('click', function (event) {
			event.preventDefault();appendSponsor('plat');
		});
		$('.gold-content-btn').on('click', function (event) {
			event.preventDefault();appendSponsor('gold');
		});
		$('.silv-content-btn').on('click', function (event) {
			event.preventDefault();appendSponsor('silv');
		});
		$('.bron-content-btn').on('click', function (event) {
			event.preventDefault();appendSponsor('bron');
		});
		$('.med-content-btn').on('click', function (event) {
			event.preventDefault();appendSponsor('med');
		});
		$('.session-content-btn').on('click', function (event) {
			event.preventDefault();appendSession('med');
		});
		$('.takeaways-content-btn').on('click', function (event) {
			event.preventDefault();appendTakeaways('takeaways');
		});
	}

	appendSponsorsInitialize();

	$(document).on('click', '.delete-session-btn', function (event) {
		event.preventDefault();
		event.target.closest('.sess-cont').remove();

		// var url = window.location.href;
		var lastPart = url.substr(url.lastIndexOf('/') + 1);
		var sessionDeleteId = $(event.target.closest('.sess-cont')).find('.session-id')[0];
...
$(document).ready(function () {
	...
	$('.btn-delete-user').on('click', function (event) {
		...
		var userDeleteId = event.target.id;
...
$(document).ready(function () {
	...
	$('.btn-delete-article').on('click', function (article) {

		var articleDeleteId = article.target.id;
...
$(document).ready(function () {
	...
	$('.new-tag').on('submit', function (tag) {
		tag.preventDefault();

		ajaxTemplate('/admin-tags/new', 'POST', newTagData($('.new-tag')), successTag, 'json');
	});

	$('.edit-tag').on('submit', function (tag) {
		tag.preventDefault();

		var tagId = $('#edit-tag-id').val();
	...
	$('.btn-delete-tag').on('click', function (tag) {

		var tagDeleteId = tag.target.id;
...
$(document).ready(function () {
	...
	$('.new-hub').on('submit', function (event) {
		event.preventDefault();

		ajaxTemplate('/admin-hubs/new', 'POST', newHubData($('.new-hub')), successHub, 'json');
	});

	$('.edit-hub').on('submit', function (event) {
		event.preventDefault();

		var hubId = $('#edit-hub-id').val();
	...
	$('.btn-delete-hub').on('click', function (event) {

		var hubDeleteId = event.target.id;
...
$(document).ready(function () {
	...
	$('.edit-customize').on('submit', function (customize) {

		customize.preventDefault();

		var customizeId = $('#edit-customize-id').val();
	...
	$('.btn-delete-customize').on('click', function (customize) {

		var customizeDeleteId = customize.target.id;
